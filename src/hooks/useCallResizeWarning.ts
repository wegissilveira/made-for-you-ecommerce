...
const useCallResizeWarning = (
   containerRef: React.RefObject<HTMLDivElement>
) => {
   const [openToastify, setOpenToastify] = useState(false);
   const [resize, setResize] = useState(true);

   const callResizeAlert = () => {
      // alert('Atualize a página para que todos os componentes se ajustem às novas dimensões. Essa mensagem não aparecerá novamente nesta sessão.')
